import { StarIcon } from "lucide-react"
import { Barberclock } from "../generated/prisma/client"
import { Badge } from "./ui/badge"
import { Button } from "./ui/button"
import { Card, CardContent } from "./ui/card"
import Image from "next/image"
import Link from "next/link"

interface BarberclockItemProps {
  barberclock: Barberclock
}

const BarberclockItem = ({ barberclock }: BarberclockItemProps) => {
  return (
    <Card className="min-w-41.75 rounded-2xl">
      <CardContent className="p-0 px-1 pt-1">
        <div className="relative h-39.75 w-full">
          <Image
            alt={barberclock.name}
            fill
            className="rounded-2xl object-cover"
            src={barberclock.imageUrl}
          />

          <Badge
            className="absolute top-2 left-2 space-x-1"
            variant={"secondary"}
          >
            <StarIcon size={12} className="fill-primary text-primary" />
            <p className="text-xs font-semibold">5,0</p>
          </Badge>
        </div>

        <div className="px-1 py-3">
          <h3 className="truncate font-semibold">{barberclock.name}</h3>
          <p className="text-grey-400 truncate text-sm">
            {barberclock.address}
          </p>
          <Button variant="secondary" className="mt-3 w-full" asChild>
            <Link href={`/barberclocks/${barberclock.id}`}>Reservar</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
  )
}

export default BarberclockItem
